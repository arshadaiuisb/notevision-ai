...
.info-box{background:#1a1d2e;border:1px solid #2a2d3e;border-radius:8px;
          padding:12px;color:#b0b3cc;font-size:.88rem;line-height:1.6}
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Be engaging, informative, and comprehensive."
                )
            },
            {
                "role": "user",
...
def make_slide_image(slide, idx, total, W=1280, H=720):
    ...
    draw.text((80, H-24),
              "Generated by NoteVision AI — Powered by Groq & Llama 3",
              fill="#4a4e6a", font=fs)
    return img
...
st.markdown(
    '<p class="subtitle">AI-powered audio & video overview generator — '
    'Powered by Groq + Llama 3 (Free)</p>',
    unsafe_allow_html=True)
...
with st.sidebar:
    ...
    api_key = get_api_key()
    if not api_key:
        api_key = st.text_input("Paste your Groq API key",
                                type="password",
                                placeholder="gsk_...")
        if api_key:
            st.session_state["groq_key"] = api_key
        st.markdown("""<div class="info-box">
        🆓 Get a <b>completely free</b> key at:<br><br>
        <b>console.groq.com</b><br><br>
        1. Sign up with Google<br>
        2. Click API Keys<br>
        3. Create & copy key (gsk_...)
...
if gen_btn:
    if not api_key:
        ...
        try:
            # Script
            prog.progress(10, text="🤖 Llama 3 is writing your script…")
            ...
            # Audio
            prog.progress(40, text="🎙 Generating MP3 audio narration…")
            if TTS_OK:
            ...
            if "401" in err or "invalid_api_key" in err.lower() or "auth" in err.lower():
                st.error("❌ Invalid Groq API key. Copy it again from console.groq.com")
